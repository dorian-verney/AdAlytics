<script>
    ...
    export let text = "Perfect for learning how the framework works, \
                       prototyping a new idea, or creating a demo to share online.";
    export let href = "/";
    
    $: logoSrc = type === "Text" ? logoText : type === "Image" ? logoImage : logoVideo;
</script>

<!-- <div class="flex min-h-screen w-full items-center justify-center bg-gray-200"> -->
<!-- Cards -->
<div class="relative flex-1 min-w-[280px] max-w-[400px] rounded-lg 
          bg-white p-3 text-gray-800 hover:scale-105 shadow-lg hover:shadow-purple-500/50 transition-all duration-200">
    <img src={logoSrc.src} alt="{type} logo" class="absolute top-3 right-3 h-8 w-8" />
    <h2 class="text-2xl font-bold">{type} Analysis.</h2>
    <p class="pt-2">{text}</p>
    <hr class="my-6 w-full border-gray-300" />
    <p class="mb-1 font-semibold">
        <a href={href} class="underline underline-offset-3 hover:font-bold
                            dark:text-white">
            Try Now. &rarr;
        </a>
    </p>
